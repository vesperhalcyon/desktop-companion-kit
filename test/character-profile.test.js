...
test('profile sanitizer bounds text, colors, arrays, and avatar paths', () => {
  const profile = sanitizeProfile({
    ...
    idleLines: [' One ', '', 'Two'],
    clickLines: ['Click'],
    dreamLines: [' Moon road ', '', 'Night library'],
  ...
  assert.deepEqual(profile.dreamLines, ['Moon road', 'Night library']);
...
test('local profile merge preserves unspecified public defaults', () => {
  ...
  assert.deepEqual(merged.dreamLines, DEFAULT_PROFILE.dreamLines);
});
