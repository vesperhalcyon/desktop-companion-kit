...
function replyToUser(message, context, profile = {}) {
  ...
  if (/\b(watch with me|movie mode|watch mode|watch this)\b/.test(lower)) {
    return response('watch', 'Put it in front of me. I will watch, and I reserve the right to have opinions.');
  }
  if (/\b(stop watching|leave movie mode|end watch mode)\b/.test(lower)) {
    return response('stopWatch', 'Movie eye closed. The rest of the desktop is none of my business.');
  }
