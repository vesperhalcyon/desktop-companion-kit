...
function sanitizeConfig(input = {}) {
  ...
  const config = {
    ...
    provider: String(input.provider || DEFAULT_CONFIG.provider).trim().toLowerCase(),
    model: String(input.model || DEFAULT_CONFIG.model).trim().toLowerCase(),
  ...
  assertDeepSeekFlash(config);
  return config;
}

function assertDeepSeekFlash(config) {
  if (config.provider !== 'deepseek' || config.model !== 'deepseek-v4-flash') {
    throw new Error('Desktop runtime text must use DeepSeek Flash');
  }
  const invocation = [config.command, ...config.args].join(' ').toLowerCase();
  if (/\b(?:codex|openai)\b/.test(invocation)) {
    throw new Error('Codex/OpenAI runtime model routes are forbidden');
  }
  const selectedModels = config.args.flatMap((value, index, args) => {
    if (value === '--model') return args[index + 1] ? [args[index + 1]] : [];
    if (value.startsWith('--model=')) return [value.slice('--model='.length)];
    return [];
  });
  if (config.enabled && (selectedModels.length !== 1
      || selectedModels[0] !== 'deepseek/deepseek-v4-flash')) {
    throw new Error('Enabled runtime text must explicitly select deepseek/deepseek-v4-flash');
  }
}
