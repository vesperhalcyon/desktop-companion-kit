...
function sanitizeConfig(input = {}) {
  const config = {
    ...
    model: String(input.model || DEFAULT_CONFIG.model).trim(),
    modality: String(input.modality || DEFAULT_CONFIG.modality).trim().toLowerCase(),
  ...
  assertNativeMiniMax(config);
  return config;
}

function assertNativeMiniMax(config) {
  if (config.provider !== 'minimax' || config.model !== 'MiniMax-M3'
      || config.modality !== 'native-video') {
    throw new Error('Watch perception must use MiniMax M3 native video');
  }
  const invocation = [config.command, ...config.args].join(' ').toLowerCase();
  if (invocation.includes('--fallback-keyframes')) {
    throw new Error('Watch perception may not fall back to extracted keyframes');
  }
  if (/\b(?:codex|openai)\b/.test(invocation)) {
    throw new Error('Codex/OpenAI vision routes are forbidden');
  }
}
